...
function Drawing() {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (
        brushSizeInputRef.current &&
        !brushSizeInputRef.current.contains(e.target) &&
  ...
  const clearCanvas = () => {
    ...
    let blankState = context.getImageData(
      0,
      0,
      contextRef.current.canvas.width,
      contextRef.current.canvas.height
  ...
  return (
    <div style={containerStyle}>
      <div style={canvasContainerStyle}>
        {!displayDoneOptions && (
          <button onClick={undo} style={undoButtonStyle}>
            <MdOutlineUndo />
          </button>
        )}
        {!displayDoneOptions && (
          <button onClick={clearCanvas} style={clearButtonStyle}>
            <FaTrashAlt />
          </button>
        )}
        {!displayDoneOptions && (
          <button onClick={redo} style={redoButtonStyle}>
            <MdOutlineRedo />
          </button>
        )}
        <canvas
        ...
        <div ref={colorPickerRef} style={colorPickerContainerStyle}>
          {!displayDoneOptions && !displayColorPicker && (
            <BsFillPaletteFill
              onClick={() => {
                setShowBrushSizeInput(false);
        ...
        <div ref={brushSizeInputRef} style={brushSizeInputStyle}>
          {!displayDoneOptions && !showBrushSizeInput && (
            <BsFillBrushFill
              onClick={(e) => {
                setDisplayColorPicker(false);
                setShowBrushSizeInput((prev) => !prev);
              }}
            />
          )}
          {showBrushSizeInput && (
        ...
        {!displayDoneOptions && (
          <button
            onClick={() => setDisplayDoneOptions(true)}
        ...
        {displayDoneOptions && (
          <div style={doneButtonsContainerStyle}>
            <button onClick={handleSaveClick} style={doneButtonsStyle}>
              SAVE
            </button>
            <button onClick={quit} style={doneButtonsStyle}>
              QUIT
            </button>
            <button
              onClick={() => setDisplayDoneOptions(false)}
              style={doneButtonsStyle}
            >
              BACK
            </button>
          </div>
